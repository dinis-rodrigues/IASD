...
initial = None # place here the initial state (or None)
airplanes = []
airports = []
legs = []
possible_actions = []
    ...
    for line in lines:
        ...
        if line[0] == 'A':
            try:
                ...
                airports.append(Airport(calculate_time(int(t_open)), calculate_time(int(t_close)), code))
            except:
        ...
        if line[0] == 'P':
            try:
                ...
                code, classe = words[1:]
                airplanes.append(Airplane(classe=classe, code=code))
            except:
